...
# 从指纹数据库导入数据获取函数和新的DID管理函数
from fingerprint_database import get_fingerprint_data, get_did_info, update_did


class HeartbeatReporter:
    """
    模拟B站的心跳上报 (reportV2)，以实现播放量增长。

    该过程分为两次上报：
    1. 开始心跳 (start heartbeat): 在播放开始时发送，上报播放的初始状态。
    2. 结束心跳 (end heartbeat): 在播放结束时发送，上报累积的播放数据。
    """
    ...
    def _generate_payload(self, start_ts,
                          actual_played_time,is_start: bool, played_time: int = 0, total_time: int = 0, paused_time: int = 0,
                          video_duration: int = 0, max_progress: int = 0, ) -> dict:
        """
        根据是“启动”还是“结束”心跳来生成请求体。
        :param start_ts:
        :param actual_played_time:
        """
        payload = {
            # --- 视频/播放相关参数 ---
            'aid': self.aid,
            ...
            'sid': "0",
            'mid': "0",
            'video_duration': str(video_duration),  # 视频总时长
            'quality': "32",
            'user_status': "0",
            'play_mode': "1",
            "epid_status": "0",
    ...
    def report_end(self, played_time: int, video_duration: int):
        """
        上报“结束”心跳。
        :param played_time: 本次会话实际播放的时长（秒）。
        :param video_duration: 视频的总时长（秒）。
        """
        print(f"\n--- [2/2] 发送 '结束' 心跳 (播放时长: {played_time}s) ---")
        # 总上报时长=播放时长，最大进度=播放时长
...
def _get_new_did(aid: str, cid: str):
    """
    发送一个不带 'did' 的请求来获取新的 'rpdid'。
    这个函数的大部分逻辑复制自 click() 函数，以确保所有其他参数和签名过程一致。
    """
    # 获取指纹数据，但忽略其中的 'did'
...
def click(bvid):
    ...
    headers = {
        ...
        "x-bili-metadata-ip-region": "CN",
        "x-bili-network-bin": fp_data["x-bili-network-bin"],
        "x-bili-ticket": fp_data.get("x-bili-ticket", ""), # TODO JWT票据
        # 为每个请求生成一个新的trace-id
        "x-bili-trace-id": f"{uuid.uuid4().hex.replace('-', '')}:{uuid.uuid4().hex[16:]}:0:0"
    }

    # 4. 构建POST请求体
    #  一些参数是动态的或与上下文相关的 (如时间戳、操作ID等)
    ftime = int(time.time()) - 1000  # 示例，基于原始逻辑
    stime = int(time.time())
    # 以下参数是占位符或从抓包请求中获得的值
    report_flow_data = "%7B%22flow_card_type%22%3A%22av%22%2C%22flow_source%22%3A%22recent_play_online_swing%22%7D"
    track_id = "all_49.router-pegasus-2206820-zf9n6.1757930398249.375"

    # 使用元组列表来保持参数顺序，然后拼接成字符串
    data_params = [
        ("aid", aid),
        ("auto_play", "0"),
        ("build", fp_data['build']),
...
def sign_sha256(data_string: str) -> str:
    """为数据字符串计算SHA256签名 (sign)"""
    salt = "9cafa6466a028bfb" # 这是固定的加盐值
...
def sign_aes(data_string: str) -> str:
    ...
    iv = b"77b07a672d57d64c"   # AES初始化向量 (IV)
    
    # 用于签名的数据是 POST请求体 + SHA256签名
...
if __name__ == '__main__':
    ...
    watch_duration=video_duration=int(input("请输入视频（观看）时长: "))
    ...
    for i in range(1, 101):  # 循环100次
        ...
        print(f"<<< 本次点击完成，随机等待 {sleep_duration:.2f} 秒后继续...")
